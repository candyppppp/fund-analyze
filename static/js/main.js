...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchAllBuyRecords().then(records => {
        ...
        // 无论云端记录是否有数据，都尝试重新渲染（确保排序正确）
        // 如果基金列表还没加载完，cached 为空，renderFunds 不会执行
    ...
    return new Promise((resolve, reject) => {
        ...
        if (cachedFunds && cachedFunds.length > 0) {
            ...
            if (isNavSettled()) {
                ...
                // 结算后：检查 nav_updated_at 是否是今天，不是则丢弃缓存重拉
                const _sampleFund = cachedFunds[0];
                const _cachedDate = _sampleFund && _sampleFund.nav_updated_at
                    ? String(_sampleFund.nav_updated_at).slice(0, 10) : '';
        ...
        fundItem.innerHTML = `
            ...
            <div class="fund-performance">
                    ...
                    <div class="fund-return-container">
                        ...
                        `}
                        <div class="fund-return-label">${settled ? '--' : 'Real-time'}</div>
                    </div>
