...
class BloggerTracker {
    ...
    _renderShell(container) {
        ...
        container.innerHTML = '<div id="bt-root">'
            ...
            + '</div>'
            + '<div class="bt-seg">'
            + '<button class="bt-seg-btn' + (r==='1D'?' on':'') + '" data-range="1D" title="近1个交易日">1D</button>'
            + '<button class="bt-seg-btn' + (r==='3D'?' on':'') + '" data-range="3D" title="近3个交易日">3D</button>'
            + '<button class="bt-seg-btn' + (r==='7D'?' on':'') + '" data-range="7D" title="近7个交易日">7D</button>'
            + '</div></div></div>'
